...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    <div class="container mx-auto rounded-lg bg-white p-4 shadow">
        <div class="mb-4 flex items-center justify-between">
            ...
            <a href="Ventas.aspx" class="border-secondary text-secondary rounded border px-4 py-2 font-semibold transition hover:bg-secondary hover:text-white">Cancelar</a>
        </div>

        <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
                <label class="text-primary mb-1 block text-sm font-medium">Tipo de cliente</label>
                <asp:RadioButtonList ID="rblCliente" runat="server" RepeatDirection="Horizontal" CssClass="text-primary flex gap-2" OnSelectedIndexChanged="rblCliente_SelectedIndexChanged" AutoPostBack="true">
            ...
            <asp:Panel ID="pnlClienteComercial" runat="server" Visible="false" CssClass="mb-2 rounded-lg border border-gray-200 bg-gray-50 p-2">
                <div class="flex flex-col md:flex-row md:gap-2">
                    <div class="mb-2 flex-1 md:mb-0">
                        <label for="ddlClientes" class="text-primary mb-1 block text-xs font-medium">Cliente:</label>
                        <asp:DropDownList ID="ddlClientes" runat="server" CssClass="w-full rounded-md border border-gray-300 px-2 py-1 shadow-sm focus:ring-primary focus:border-primary focus:outline-none"></asp:DropDownList>
                    </div>
                </div>
            </asp:Panel>
            <div>
                <label for="ddlMetodoPago" class="text-primary mb-1 block text-xs font-medium">Método de Pago:</label>
                <asp:DropDownList ID="ddlMetodoPago" runat="server" CssClass="w-full rounded-md border border-gray-300 px-2 py-1 shadow-sm focus:ring-primary focus:border-primary focus:outline-none"></asp:DropDownList>
            </div>
        </div>
        <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
                <label class="text-primary mb-1 block text-xs font-medium">Tipo de Venta:</label>
                <asp:RadioButtonList ID="rdbEnLocal" runat="server" RepeatDirection="Horizontal" CssClass="text-primary flex gap-2">
            ...
            <div>
                <label for="lblFecha" class="text-primary mb-1 block text-xs font-medium">Fecha de Venta:</label>
                <asp:Label ID="lblFecha" runat="server" Text="Fecha: " CssClass="text-primary text-base font-semibold"></asp:Label>
            </div>
        </div>
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
            <div class="rounded bg-gray-50 p-3">
                <label for="lblSubtotal" class="text-primary block text-xs">Subtotal:</label>
                <asp:Label ID="lblSubtotal" runat="server" Text="0.00" CssClass="text-primary text-lg font-bold"></asp:Label>
            </div>
            <div class="rounded bg-gray-50 p-3">
                <label for="lblTotal" class="text-primary block text-xs">Total de Venta:</label>
                <asp:Label ID="lblTotal" runat="server" Text="0.00" CssClass="text-primary text-xl font-bold"></asp:Label>
            </div>
            <div class="rounded bg-gray-50 p-3">
                <label for="txtMontoCliente" class="text-primary block text-xs">Monto Pagado por Cliente:</label>
                <asp:TextBox ID="txtMontoCliente" runat="server" TextMode="Number"
                    CssClass="w-full rounded-md border border-gray-300 px-2 py-1 shadow-sm focus:ring-primary focus:border-primary focus:outline-none"
                    onkeyup="calcularCambio();" />
            </div>
            <div class="rounded bg-gray-50 p-3">
                <label for="lblCambio" class="text-primary block text-xs">Cambio:</label>
                <asp:Label ID="lblCambio" runat="server" Text="0.00" CssClass="text-secondary text-lg font-bold"></asp:Label>
            </div>
        </div>

        <div class="flex justify-end gap-2">
            <asp:Button ID="btnGuardar" runat="server" Text="Guardar Cambios" OnClick="btnGuardar_Click"
                CssClass="bg-primary rounded px-6 py-2 font-semibold text-white shadow transition hover:bg-gray-900" />
        </div>

        <asp:Button ID="btnCompletarVenta" runat="server" Text="Completar Venta" Visible="false"
            CssClass="hidden rounded bg-green-600 px-6 py-2 font-semibold text-white shadow hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2" />
