...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <div class="flex w-full flex-col justify-between py-4">
        <div class="mb-4 flex items-center justify-between">
            <h2 class="text-primary text-3xl font-extrabold">Gestion de Usuarios</h2>
            <asp:Button CssClass="bg-primary mt-2 inline-block rounded px-4 py-2 font-semibold text-white shadow transition-colors hover:bg-primary/85 hover:cursor-pointer focus:ring-primary focus:outline-none focus:ring-2 focus:ring-offset-2" Text="Crear Usuarios" runat="server" ID="btnRegistrarUsuario" OnClick="btnRegistrarUsuario_Click" />
        </div>
    </div>
    ...
    <div class="flex rounded-lg bg-white p-4">
        <div class="flex w-full flex-col justify-between">
            <asp:Repeater runat="server" ID="rptUsuarios" OnItemCommand="rptUsuarios_ItemCommand">
                <HeaderTemplate>
                    <table class="w-full table-auto border-collapse overflow-hidden rounded-lg shadow" id="tbUsuarios">
                        <thead>
                ...
                <ItemTemplate>
                    <tr>
                        <td class="text-secondary px-4 py-2 font-semibold"><%# Eval("Id") %></td>
                        <td class="px-4 py-2"><%# Eval("Nombre") %></td>
                        <td class="px-4 py-2">••••••••</td>
                        <td class="px-4 py-2"><%# Eval("Rol.Nombre") %></td>
                        <td class="px-4 py-2"><%# Eval("Negocio.Nombre") %></td>
                        <td class="px-4 py-2">
